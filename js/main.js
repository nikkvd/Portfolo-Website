...
function openModal(modalId) {
    if (isModalOpen) return; // Prevent opening a new modal if one is already open
...
// Prevent clicking on other modals while one is open
document.querySelectorAll('.portfolio-item').forEach(item => {
    item.addEventListener('click', (event) => {
        if (isModalOpen) {
            event.stopPropagation(); // Stop event from triggering another modal
        }
    });
});
...
const itemWidth = container.querySelector('.portfolio-item').offsetWidth + 20;

function scrollPortfolio(direction) {
    const containerWidth = document.querySelector('.portfolio-wrapper').offsetWidth;
    const totalItems = container.querySelectorAll('.portfolio-item').length;
    const maxScroll = (itemWidth * totalItems) - containerWidth;

    // Check if scrolling is possible in the given direction
    if ((direction === 1 && scrollPosition < maxScroll) || (direction === -1 && scrollPosition > 0)) {
        scrollPosition += direction * containerWidth;

        // Ensure it does not scroll past the limits
